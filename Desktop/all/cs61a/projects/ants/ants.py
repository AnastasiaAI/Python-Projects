...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.scared_once and self.direction == 'backwards':
            if self.place.entrance is not colony.hive:
                self.move_to(self.place.entrance)
        elif self.direction == 'forward': # or self.scared_once == True:
        # END EC
            if self.blocked():
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while examine_place is not hive and nearest_place == None and count <= self.max_range:
            if count < self.min_range or not examine_place.bees:
                examine_place = examine_place.entrance
                count += 1
            else:
                nearest_place = examine_place
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digest:
            self.digest -= 1
        elif len(self.place.bees) > 0: #hmmmmm
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        tank_container = self.place.bees[:]
        for bee in tank_container[:]:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if not self.true_queen: # fake quuens die
            self.reduce_armor(self.armor)
        else: # true queen
            self.throw_at(self.nearest_bee(colony.hive)) # attacks
            q_location = self.place
            while q_location.exit:
                q_location = q_location.exit
                if q_location.ant: # = self.place.exit.ant
                    if q_location.ant.is_container and q_location.ant.contained_ant and (q_location.ant.contained_ant not in self.boosted_bees):
                        q_location.ant.contained_ant.damage = q_location.ant.contained_ant.damage * 2
                        self.boosted_bees.append(q_location.ant.contained_ant)
                    if q_location.ant not in self.boosted_bees:
                        q_location.ant.damage = q_location.ant.damage*2
                        self.boosted_bees.append(q_location.ant)
        # END Problem 13
...
def apply_effect(effect, bee, duration): #slow overrides scare
    ...
    def inner(colony):
        nonlocal duration
        if bee.duration and effect == make_slow:
            bee.duration -= 1
        if duration > 0:
            new_action(colony)
            duration -= 1
        elif duration == 0 and bee.duration == 0:
            bee.direction = 'forward'
            original_action(colony)
        elif duration == 0 and bee.duration:
            bee.action(colony)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target.scared_once:
            return
        else:
            return apply_effect(make_scare, target, 2)# back away / (stay if at the back) + still for 2 turns
        # END Problem EC
