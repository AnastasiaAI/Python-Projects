...
namespace
{
	const float k_rotateViewportAngle = 15.f;
}
...
enum class RasterazeTriangleLineState
{
	INIT
	,ENTER_TRIANGLE
	,LEAVE__TRIANGLE
 ...
 */
static const string level_strings[] = { "level zero", "nearest level", "bilinear level interpolation", "anisotropic nearest level interpolation", "anisotropic linear level interpolation" };
...
void DrawRend::keyboard_event( int key, int event, unsigned char mods ) {
  ...
  switch( key ) {
	...
	case '[':
		rotate_view(k_rotateViewportAngle);
		redraw();
		break;
...
{
	Matrix3x3& m = svg_to_ndc[current_svg];
	Matrix3x3 rorMatrix = rotate(deg);
	Vector2D translateToCenter(svgs[current_svg]->width / 2, svgs[current_svg]->height / 2);
	Matrix3x3 tm = translate(translateToCenter.x, translateToCenter.y) * rorMatrix * translate(translateToCenter.x, translateToCenter.y).inv();
	svg_to_ndc[current_svg] = m * tm;
}
...
#ifdef FAST_RASTERIZATION
	Vector2D vMinX;
	if (v0.x == xMin) vMinX = v0;
	if (v1.x == xMin) vMinX = v1;
	if (v2.x == xMin) vMinX = v2;

	int currentInc_j = vMinX.y;
	int currentDec_j = vMinX.y;

	for (int i = xMin; i < xMax; ++i)
	{
		{
			RasterazeTriangleLineState state = RasterazeTriangleLineState::INIT;

			for (int j = currentInc_j; j < yMax; ++j)
			{
				SampleBuffer & sampleFragment = samplebuffer[j][i];
				const size_t sps = sampleFragment.samples_per_side;

				for (int s_i = 0; s_i < sps; s_i++)
				{
					for (int s_j = 0; s_j < sps; s_j++)
					{
						double shift = 1. / (double)sps;

						Vector2D v((double)i + shift * (s_i + 1), (double)j + shift * (s_j + 1));

						double d0 = cross(v0 - v1, v - v0);
						double d1 = cross(v1 - v2, v - v1);
						double d2 = cross(v2 - v0, v - v2);

						if ((d0 >= 0. && d1 >= 0. && d2 >= 0.) || (d0 <= 0. && d1 <= 0. && d2 <= 0))
						{
							if (state == RasterazeTriangleLineState::INIT)
							{
								state = RasterazeTriangleLineState::ENTER_TRIANGLE;
								currentInc_j = j;
							}

							if (tri)
							{
								double d = cross(v0 - v1, v2 - v1);
								Vector3D bary(d0 / d, d1 / d, d2 / d);

								SampleParams sampleParam;
								sampleParam.psm = psm;
								sampleParam.lsm = lsm;

								Vector2D vdx((double)i + shift * (s_i + 1), (double)j + shift * (s_j + 2));
								double dx0 = cross(v0 - v1, vdx - v0);
								double dx1 = cross(v1 - v2, vdx - v1);
								double dx2 = cross(v2 - v0, vdx - v2);

								Vector2D vdy((double)i + shift * (s_i + 2), (double)j + shift * (s_j + 1));
								double dy0 = cross(v0 - v1, vdy - v0);
								double dy1 = cross(v1 - v2, vdy - v1);
								double dy2 = cross(v2 - v0, vdy - v2);

								Vector3D dxBary(dx0 / d, dx1 / d, dx2 / d);
								Vector3D dyBary(dy0 / d, dy1 / d, dy2 / d);

								sampleFragment.fill_color(s_i, s_j, tri->color(bary, dxBary, dyBary, sampleParam));
							}
							else
							{
								sampleFragment.fill_color(s_i, s_j, color);
							}
						}
						else
						{
							if (state == RasterazeTriangleLineState::ENTER_TRIANGLE)
							{
								state = RasterazeTriangleLineState::LEAVE__TRIANGLE;
							}
						}
					}
				}

				if (state == RasterazeTriangleLineState::LEAVE__TRIANGLE)
				{
					break;
				}
			}
		...
		{
			RasterazeTriangleLineState state = RasterazeTriangleLineState::INIT;
			for (int j = currentDec_j; j >= yMin; --j)
			{
				SampleBuffer & sampleFragment = samplebuffer[j][i];
				const size_t sps = sampleFragment.samples_per_side;
				...
				for (int s_i = 0; s_i < sps; s_i++)
				{
					for (int s_j = 0; s_j < sps; s_j++)
					{
						double shift = 1. / (double)sps;
						 ...
						 // Part2 EC  Uniform Sampling + Jitter
						double jitter_x = ((drand() - 0.5) * shift) * 0.5; // 0 to 1, want -0.5 to +0.5
						double jitter_y = ((drand() - 0.5) * shift) * 0.5;

#else
						// Part2, normal Supersampling
						double jitter_x = 0; // 0 to 1, want -0.5 to +0.5
						double jitter_y = 0;
#endif // USE_JITTERING
						...
						Vector2D v((double)i + (shift * 0.5) + (shift * s_i) + jitter_x, (double)j + shift * 0.5 + shift * s_j + jitter_y);
						...
						if ((d0 >= 0. && d1 >= 0. && d2 >= 0.) || (d0 <= 0. && d1 <= 0. && d2 <= 0))
						{
							if (tri)
							{
								...
								// Vector2D vdx((double)i + shift * (s_i + 1), (double)j + shift * (s_j + 2));
								double vdx_coord_x = (double)i + (shift * 0.5) + (shift * s_i);
								double vdx_coord_y = (double)j + shift * 0.5 + shift * (s_j + 1);
								Vector2D vdx(vdx_coord_x + jitter_x, vdx_coord_y + jitter_y);
								double dx0 = cross(v0 - v1, vdx - v0);
								double dx1 = cross(v1 - v2, vdx - v1);
								double dx2 = cross(v2 - v0, vdx - v2);
								...
								double vdy_coord_x = (double)i + (shift * 0.5) + (shift * (s_i + 1));
								double vdy_coord_y = (double)j + shift * 0.5 + shift * s_j;
								Vector2D vdy(vdy_coord_x + jitter_x, vdy_coord_y + jitter_y);
								double dy0 = cross(v0 - v1, vdy - v0);
								double dy1 = cross(v1 - v2, vdy - v1);
								double dy2 = cross(v2 - v0, vdy - v2);
