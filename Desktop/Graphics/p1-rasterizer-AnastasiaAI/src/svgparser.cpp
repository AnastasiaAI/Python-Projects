...
void SVGParser::parseImage( XMLElement* xml, Image* image ) {
  ...
  RipLevel rip_start;
  rip_start.width  = width;
  rip_start.height = height;
  rip_start.texels = pixels_no_alpha;

  // add to svg
  image->tex.width  = rip_start.width;
  image->tex.height = rip_start.height;

  image->tex.ripmap.push_back(std::vector< RipLevel >(1, rip_start));
}
