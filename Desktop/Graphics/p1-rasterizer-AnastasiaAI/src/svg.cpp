...
Color ColorTri::color(Vector3D p_bary, Vector3D p_dx_bary, Vector3D p_dy_bary, SampleParams sp) {
  // Part 4: Fill this in.
	Color color;
	color.r = p_bary[1] * p0_col.r + p_bary[2] * p1_col.r + p_bary[0] * p2_col.r;
	color.g = p_bary[1] * p0_col.g + p_bary[2] * p1_col.g + p_bary[0] * p2_col.g;
	color.b = p_bary[1] * p0_col.b + p_bary[2] * p1_col.b + p_bary[0] * p2_col.b;
	return color;
}
...
Color TexTri::color(Vector3D p_bary, Vector3D p_dx_bary, Vector3D p_dy_bary, SampleParams sp) {
  ...
  // Part 6: Fill this in with trilinear sampling as well.^
	...
	sp.p_uv.x = p_bary[1] * p0_uv.x + p_bary[2] * p1_uv.x + p_bary[0] * p2_uv.x;
	sp.p_uv.y = p_bary[1] * p0_uv.y + p_bary[2] * p1_uv.y + p_bary[0] * p2_uv.y;

	sp.p_dx_uv.x = p_dx_bary[1] * p0_uv.x + p_dx_bary[2] * p1_uv.x + p_dx_bary[0] * p2_uv.x;
	sp.p_dx_uv.y = p_dx_bary[1] * p0_uv.y + p_dx_bary[2] * p1_uv.y + p_dx_bary[0] * p2_uv.y;

	sp.p_dy_uv.x = p_dy_bary[1] * p0_uv.x + p_dy_bary[2] * p1_uv.x + p_dy_bary[0] * p2_uv.x;
	sp.p_dy_uv.y = p_dy_bary[1] * p0_uv.y + p_dy_bary[2] * p1_uv.y + p_dy_bary[0] * p2_uv.y;
