...
private:
  ...
  static unsigned char float_to_ucahr_color(const float & color) 
  {
	  return static_cast<unsigned char>(255.f * color);
  }

  static float ucahr_to_float_color(const unsigned char & color)
  {
  ...
  struct SampleBuffer {
    ...
    void fill_color(int i, int j, Color c) {
	  ...
	  p[0] = float_to_ucahr_color(c.r);
	  p[1] = float_to_ucahr_color(c.g);
	  p[2] = float_to_ucahr_color(c.b);
      return;
    }
    ...
    Color get_pixel_color() {
		...
		for (int i = 0; i < samples_per_side; ++i)
		{
			for (int j = 0; j < samples_per_side; ++j)
			{
				const PixelColorStorage & subColor = sub_pixels[i][j];
				resColor.r += ucahr_to_float_color(subColor[0]);
				resColor.g += ucahr_to_float_color(subColor[1]);
				resColor.b += ucahr_to_float_color(subColor[2]);
			}
		}

		resColor.r /= samples_per_side * samples_per_side;
		resColor.g /= samples_per_side * samples_per_side;
		resColor.b /= samples_per_side * samples_per_side;
