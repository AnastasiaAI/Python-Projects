    ...
    void DrawRend::rasterize_triangle( float x0, float y0,
        ...
        for ( int i = xMin; i <= xMax; ++i)
        {
            for (int j = yMin; j <= yMax ; ++j)
            {
                ...
                double shift = 1. / (double)sps; // sps = 2; shift = 0.5;
                ...
                for (int s_i = 0; s_i < sps; s_i++)
                {
                    for (int s_j = 0; s_j < sps; s_j++)
                    {
                       ...
                       // Vector2D v((double)i + (shift * 0.5) + (shift * s_i), (double)j + shift * 0.5 + shift * s_j);
                        
                        
                        // Part2 EC  Uniform Sampling + Jitter
                        double jitter_x = ((drand48() - 0.5) * shift) * 0.5; // 0 to 1, want -0.5 to +0.5
                        double jitter_y = ((drand48() - 0.5) * shift) * 0.5;
